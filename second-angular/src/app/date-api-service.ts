...
export class InMemoryDataService {
  ...
  getWeather(startDate: Date, endDate: Date): Observable<WeatherData[]> {
    ...
    const rawStartMonth = (startDate.getMonth().toString().length > 1) ? startDate.getMonth() + 1 : `0${startDate.getMonth() + 1}`;
    ...
    const rawEndMonth = (endDate.getMonth().toString().length > 1) ? endDate.getMonth() + 1 : `0${endDate.getMonth() + 1}`;
    const rawEndYear = endDate.getFullYear();
    const sDate: String = `${rawStartYear}-${rawStartMonth}-${rawStartDay}`;
    const eDate: String = `${rawEndYear}-${rawEndMonth}-${rawEndDay}`;
